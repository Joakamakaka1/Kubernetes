...
public class Validator {
    public static void validateUserRegister(UserRegisterDTO userRegisterDTO) {
        if (userRegisterDTO.getEmail().isEmpty() || userRegisterDTO.getEmail() == null) {
            throw new ValidationException("El email del usuario no puede estar vacio");
        }
        if (userRegisterDTO.getUsername().isEmpty() || userRegisterDTO.getUsername() == null) {
            throw new ValidationException("El username del usuario no puede estar vacio");
        }
        if (userRegisterDTO.getPassword1().isEmpty() || userRegisterDTO.getPassword1() == null) {
            throw new ValidationException("La contraseña del usuario no puede estar vacia");
        }
        if (userRegisterDTO.getPassword2().isEmpty() || userRegisterDTO.getPassword2() == null) {
            throw new ValidationException("La contraseña repetida del usuario no puede estar vacia");
        }
